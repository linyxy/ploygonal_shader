...
float2 LTC_Coords(float cosTheta, float roughness)
{
    ...
    float2 coords = float2(roughness, theta/(0.5*3.14159));
...
//float3x3 LTC_Matrix(sampler2D texLSDMat, float2 coord)
//{
//    // load inverse matrix
//    float4 t = texture2D(texLSDMat, coord);
//    float3x3 Minv = float3x3_from_columns(
//        float3(1,     0, t.y),
//        float3(  0, t.z,   0),
//        float3(t.w,   0, t.x)
//    );
...
float3 LTC_Evaluate(float3 N, float3 V, float3 P, float3x3 Minv, float4 points[4], bool twoSided)//, sampler2D texFilteredMap
{
    ...
    Minv = mul(Minv, float3x3_from_rows(T1, T2, N));
