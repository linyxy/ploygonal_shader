...
void CalcQuadPoints(DirectX::XMFLOAT4 quad_points[4], XMFLOAT3 &rotation, XMFLOAT4 &pos, XMFLOAT3 &scale) {
	float x = XMConvertToRadians(rotation.x);
	float y = XMConvertToRadians(rotation.y);
	float z = XMConvertToRadians(rotation.z);
	XMVECTOR position = XMLoadFloat4(&pos);
	XMMATRIX scalee = XMMatrixScaling(scale.x, scale.y, scale.z);
	XMMATRIX translate =	XMMatrixTranslationFromVector(position);
	XMMATRIX rotateX = XMMatrixRotationX(x);
	XMMATRIX rotateY = XMMatrixRotationY(y);
	XMMATRIX rotateZ = XMMatrixRotationZ(z);
	XMMATRIX lightTransform = translate * rotateX * rotateY * rotateZ * scalee;
	
	XMFLOAT4 floats[4] = { XMFLOAT4(-1, 1, 0, 1 ), XMFLOAT4(1, 1, 0, 1), XMFLOAT4(1, -1, 0, 1), XMFLOAT4(-1, -1, 0, 1) };

	for (auto i : { 1,2,3,4 }) {
		XMVECTOR vector = XMLoadFloat4(&floats[i]);
		XMVECTOR res = XMVector4Transform(vector, lightTransform);
		XMStoreFloat4(&quad_points[i], res);
	}
}
...
void LitColumnsApp::UpdateMainPassCB(const GameTimer& gt)
{
	...
	mMainPassCB.ALights[0].albedo = { 1.0f, 1.0f, 1.0f, 1.0f };
	mMainPassCB.ALights[0].lightInten_twoSide = {100, 0, 0, 0};
	mMainPassCB.ALights[0].lightPosition = { 0, 0, 0, 1 };
	mMainPassCB.ALights[0].param_F0_rought_sampleCount = { 1.0f, 0.03f, 0.0f, 4.0f };
	CalcQuadPoints(mMainPassCB.ALights[0].quad_points, XMFLOAT3(0, 90, 0), mMainPassCB.ALights[0].lightPosition, XMFLOAT3(25, 25, 1));
	mMainPassCB.ALights[0].samples;
	mMainPassCB.ALights[0].viewPosition;
